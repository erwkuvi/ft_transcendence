...
function Pong() {
	...
	useEffect(() => {
		let newSocket = new WebSocket(`ws://localhost:8000/ws/game/`);
		setSocket(newSocket);

		newSocket.onopen = () => console.log('WebSocket for game connection established');
		...
		newSocket.onclose = () => {
			...
			setTimeout(() => {
				...
				newSocket = new WebSocket(`ws://localhost:8000/ws/game/`);
				setSocket(newSocket);
			}, 3000);
		};
